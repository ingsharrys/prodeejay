...
$allowedRoles = ipp_itro_get_option('roles');
if (!ipp_itro_can_manage()) {
...
for ($i = 0; $i < count($opt_name); $i++) {
    ...
    if (isset($_POST[$submitted_form]) && $_POST[$submitted_form] == 'Y') {
        ...
        ipp_update_option($opt_name[$i], $opt_val[$i]);
        
        if (isset($_POST['select_' . $opt_name[$i]])) {
            ipp_update_option('select_' . $opt_name[$i], $_POST['select_' . $opt_name[$i]]);
            
            $px_opt_val[$i] = ipp_validate_data('px_' . $opt_name[$i], $_POST['px_' . $opt_name[$i]]);
            ipp_update_option('px_' . $opt_name[$i], $_POST['px_' . $opt_name[$i]]);
            $perc_opt_val[$i] = ipp_validate_data('perc_' . $opt_name[$i], $_POST['perc_' . $opt_name[$i]]);
            ipp_update_option('perc_' . $opt_name[$i], $_POST['perc_' . $opt_name[$i]]);
        } else {
            ipp_update_option('select_' . $opt_name[$i], NULL);
        }
    } else {
        /* Read in existing option value from database */
        $opt_val[$i] = ipp_itro_get_option($opt_name[$i]);
        $px_opt_val[$i] = ipp_itro_get_option('px_' . $opt_name[$i]);
        $perc_opt_val[$i] = ipp_itro_get_option('perc_' . $opt_name[$i]);
    }
}
...
if (isset($_POST[$submitted_form]) && $_POST[$submitted_form] == 'Y') {
    ...
    /* Save the posted value in the database */
    ipp_itro_update_field('custom_html', $field_value);
    
    
    if (isset($_POST['selected_page_id'])) {
        $selected_page_id = json_encode($_POST['selected_page_id']);
        ipp_update_option('selected_page_id', $selected_page_id);
    } else {
        ipp_update_option('selected_page_id', NULL);
    }
    
    if (!empty($_POST['background_source'])) {
        ipp_update_option('background_source', $_POST['background_source']);
    }
    
    if (isset($_POST['roles'])) {
        $roles_users = json_encode($_POST['roles']);
        ipp_update_option('roles', $roles_users);
    } else {
        ipp_update_option('roles', NULL);
    }
} else {
    //WP Editor Content
    $field_value = stripslashes(ipp_itro_get_field('custom_html'));
}
...
if (isset($_POST['delete_data_hidden']) && $_POST['delete_data_hidden'] == 'Y') {
    ...
    if (isset($_POST['delete_data'])) {
        ipp_update_option('delete_data', $_POST['delete_data']);
    } else {
        ipp_update_option('delete_data', NULL);
    }
}
...
?>
    <script type="text/javascript" src="<?php echo ippitroPath . 'scripts/'; ?>jscolor/jscolor.js"></script>
            ...
            <div id="formContainer">
                ...
                <div id="genOption">
                    ...
                    <p>
                        ...
                        <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>" title="<?php _e('Display many other options', 'itro-plugin'); ?>"/>
                    </p>
                    ...
                    foreach ($roles_obj->roles as $role => $role_val) {
                        $allowedRoles = ipp_itro_get_option('roles');
                        $allowedRoles = $allowedRoles == NULL ? "" : json_decode(ipp_itro_get_option('roles'));
                    ...
                    <h4>For the users:</h4>
                    <fieldset >
                        <input type="radio" name="<?php echo $opt_name[38]; ?>" checked id="clickable_a" value="all" <?php if ($opt_val[38] == 'all') {
                            echo 'checked="checked"';
                    ...
                    <fieldset>
                        <input type="radio" id="only_selected" name="<?php echo $opt_name[18]; ?>" value="some"<?php if ($opt_val[18] == 'some') {
                            echo 'checked="checked"';
                        } ?>/> <label for="only_selected"> <?php _e("Only selected pages", 'itro-plugin'); ?> </label> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                    ...
                    <div onClick="document.getElementById('only_selected').checked = true;">
                        ...
                        <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                    ...
                    <div style="display:table; height:10px; padding-bottom:5px;">
                        <span style="float:left;"><?php _e("Popup seconds:", 'itro-plugin'); ?> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                    ...
                    <div id="<?php echo $opt_name[23]; ?>_div" style="display:table; height:10px; padding-bottom:5px;">
                        <span style="float:left;"><?php _e("Popup delay:", 'itro-plugin'); ?> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                    ...
                    <div style="display:table; height:10px; padding-bottom:5px;">
                        <span style="float:left;"><?php _e("Next visualization (hours):", 'itro-plugin'); ?> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                    ...
                    <p id="<?php echo $opt_name[29]; ?>_div">
                        <input type="checkbox" name="<?php echo $opt_name[29]; ?>" id="<?php echo $opt_name[29]; ?>" value="yes" <?php if (ipp_itro_get_option($opt_name[29]) == 'yes') {
                            echo 'checked="checked"';
                        } ?> />&nbsp;&nbsp;
                        ...
                        <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>" title="<?php _e('Countdown text at the bottom border of the popup', 'itro-plugin'); ?>">
                        ...
                        <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>" title="<?php _e('Select the countdown font color.', 'itro-plugin'); ?>">
                    ...
                    <!-- popup width !-->
                    <div style="display:table; height:10px; padding-bottom:5px;">
                        <span style="float:left;"><?php _e("Popup width:", 'itro-plugin'); ?> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                        ...
                        <input type="text" class="itro_text_input" style="<?php if (ipp_itro_get_option('select_' . $opt_name[3]) != 'px') {
                            echo 'display:none;';
                        } ?>" id="<?php echo 'px_' . $opt_name[3]; ?>" name="<?php echo 'px_' . $opt_name[3]; ?>" value="<?php echo $px_opt_val[3]; ?>" size="1">
                        <input type="text" class="itro_text_input" style="<?php if (ipp_itro_get_option('select_' . $opt_name[3]) != 'perc') {
                            echo 'display:none;';
                        ...
                        <select id="select_<?php echo $opt_name[3]; ?>" name="select_<?php echo $opt_name[3]; ?>" style="position:relative; left:7px;">
                            <option value="px" onClick="itro_enable_<?php echo $opt_name[3]; ?>('px')" <?php if (ipp_itro_get_option('select_' . $opt_name[3]) == 'px') {
                                echo 'selected="select"';
                            } ?>>px
                            </option>
                            <option value="perc" onClick="itro_enable_<?php echo $opt_name[3]; ?>('perc')" <?php if (ipp_itro_get_option('select_' . $opt_name[3]) == 'perc') {
                                echo 'selected="select"';
                            } ?>>%
                            </option>
                        </select>
                    ...
                    <div id="<?php echo $opt_name[17]; ?>_div" style="display:table; height:10px; padding-bottom:5px;">
                        <span style="float:left;"><?php _e("Popup height:", 'itro-plugin'); ?> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                        ...
                        <input type="text" class="itro_text_input" style="<?php if (ipp_itro_get_option('select_' . $opt_name[17]) != 'px') {
                            echo 'display:none;';
                        } ?>" id="<?php echo 'px_' . $opt_name[17]; ?>" name="<?php echo 'px_' . $opt_name[17]; ?>" value="<?php echo $px_opt_val[17]; ?>" size="1">
                        <input type="text" class="itro_text_input" style="<?php if (ipp_itro_get_option('select_' . $opt_name[17]) != 'perc') {
                            echo 'display:none;';
                        ...
                        <select id="select_<?php echo $opt_name[17]; ?>" name="select_<?php echo $opt_name[17]; ?>" style="position:relative; left:7px;">
                            <option value="px" onClick="itro_enable_<?php echo $opt_name[17]; ?>('px')" <?php if (ipp_itro_get_option('select_' . $opt_name[17]) == 'px') {
                                echo 'selected="select"';
                            } ?>>px
                            </option>
                            <option value="perc" onClick="itro_enable_<?php echo $opt_name[17]; ?>('perc')" <?php if (ipp_itro_get_option('select_' . $opt_name[17]) == 'perc') {
                                echo 'selected="select"';
                            } ?>>%
                            </option>
                            <option value="auto" onClick="itro_disable_<?php echo $opt_name[17]; ?>();" <?php if (ipp_itro_get_option('select_' . $opt_name[17]) == 'auto') {
                                echo 'selected="select"';
                            } ?>>auto
                            </option>
                        </select>
                    ...
                    <!-- popup position !-->
                    <p><?php _e("Popup position:", 'itro-plugin'); ?> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                        ...
                        <select name="<?php echo $opt_name[16]; ?>" style="min-width:100px;">
                            <option value="absolute" <?php if (ipp_itro_get_option($opt_name[16]) == 'absolute') {
                                echo 'selected="select"';
                            } ?> >Absolute
                            </option>
                            <option value="fixed" <?php if (ipp_itro_get_option($opt_name[16]) == 'fixed') {
                                echo 'selected="select"';
                            } ?> >Fixed
                            </option>
                        </select>
                    ...
                    <p>
                        <input id="<?php echo $opt_name[30]; ?>" type="checkbox" name="<?php echo $opt_name[30]; ?>" value="yes" <?php if (ipp_itro_get_option($opt_name[30]) == 'yes') {
                            echo 'checked="checked"';
                        } ?> />
                    ...
                    <!-- popup top margin !-->
                    <div id="top_margin_slider" style=" <?php if (ipp_itro_get_option($opt_name[30]) == 'yes') {
                        echo 'display:none';
                    } else {
                        echo 'display:table';
                    ...
                    <a href="<?php if (ipp_itro_get_option('background_source') == NULL) {
                        echo 'javascript:void(0)';
                    } else {
                        echo ipp_itro_get_option('background_source');
                    ...
                    <input class="upload" onClick="select(); document.getElementById('yes_bg').checked=true" type="text" name="background_source" size="50" value="<?php echo ipp_itro_get_option('background_source'); ?>"/>
                    ...
                    <div id="<?php echo $opt_name[30]; ?>_div">
                        ...
                        <!-- background opacity !-->
                        <div style="display:table; height:10px; padding-bottom:5px;">
                            <span style="float:left;"><?php _e("Layer opacity", 'itro-plugin'); ?> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                        ...
                        <p><?php _e("Select image", 'itro-plugin'); ?> <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>"
                            ...
                            <select name="<?php echo $opt_name[33]; ?>" style="min-width:100px;">
                                <option value="black" <?php if (ipp_itro_get_option($opt_name[33]) == 'black') {
                                    echo 'selected="select"';
                                } ?> ><?php _e("Black", 'itro-plugin'); ?></option>
                                <option value="white" <?php if (ipp_itro_get_option($opt_name[33]) == 'white') {
                                    echo 'selected="select"';
                                } ?> ><?php _e("White", 'itro-plugin'); ?></option>
                                <option value="white_border" <?php if (ipp_itro_get_option($opt_name[33]) == 'white_border') {
                                    echo 'selected="select"';
                                } ?> ><?php _e("White with border", 'itro-plugin'); ?></option>
                                <option value="url" <?php if (ipp_itro_get_option($opt_name[33]) == 'url') {
                                    echo 'selected="select"';
                        ...
                        <p>
                            ...
                            <img style="vertical-align:super; cursor:help" src="<?php echo ippitroImages . 'question_mark.png'; ?>" title="<?php _e('Check this to disable popup on mobile devices', 'itro-plugin'); ?>">
                            &nbsp;&nbsp;&nbsp;&nbsp;
    ...
    <div id="rightColumn2">
        <div id="donateForm">
            <h3><?php _e("Like it? Leave us a 5 stars review! ;-)", "itro-plugin") ?> <a href="https://wordpress.org/support/view/plugin-reviews/itro-popup?filter=5" target="_blank"><img id="stars_review_img"
                                                                                                                                                                                           src="<?php echo ippitroImages . '5_stars.png'; ?>"></a>
            </h3>
            <p>
        ...
        <div method="POST" action="" id="debug_info" style="display:none;">
            <?php echo ipp_itro_get_serverinfo(); ?>
    ...
    <form id="delete_data" method="post" style="clear:both;">
        ...
        <input type="checkbox" id="delete_data" name="delete_data" value="yes" <?php if (ipp_itro_get_option('delete_data') == 'yes') {
            echo 'checked="checked"';
        } ?> />
