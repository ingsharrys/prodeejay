...
function ipp_itro_plugin_menu() {
    if(!ipp_itro_can_manage()){
        return;
    }
    add_options_page('Popup Plugin Options', 'ITRO Popup', 'read', ippRootPath . 'admin/popup-admin.php', '');
}
function ipp_itro_can_manage() {
    ...
    $selected_roles = json_decode(ipp_itro_get_option('roles'));
...
function ipp_itro_init() {
    /* -----load sample popup settings */
    if (get_option("itro_curr_ver") == NULL) {
	/* --------- initialize database */
	ipp_itro_db_init();

	ipp_update_option('popup_time', 20);
	ipp_update_option('popup_delay', 0);
	ipp_update_option('cookie_time_exp', 0);
	ipp_update_option('popup_background', '#FFFFFF');
	ipp_update_option('popup_border_color', '#F7FF00');
	ipp_update_option('px_popup_width', 600);
	ipp_update_option('px_popup_height', 0);
	ipp_update_option('show_countdown', 'yes');
	ipp_update_option('auto_margin_check', 'yes');
	ipp_update_option('select_popup_width', 'px');
	ipp_update_option('select_popup_height', 'auto');
	ipp_update_option('popup_bg_opacity', 0.40);
	ipp_update_option('opaco_bg_color', '#8A8A8A');
	ipp_update_option('popup_position', 'fixed');
	ipp_update_option('popup_border_width', 3);
	ipp_update_option('popup_border_radius', 8);
	ipp_update_option('popup_padding', 2);
	ipp_update_option('page_selection', 'none');

	switch (get_locale()) {
	    case 'en_US':
		$welcome_text = '<h1 style="text-align: center;"><span style="color: #000000; font-size: 20px;">Hello, this is a pop-up sample.</span></h1><p style="text-align: center;"><span style="color: #000000; font-size: 20px;">The basic stetting to get started are: Popup height, Popup time, Next visualization, Popup border color, Popup background.</span></p><p style="text-align: center;"><span style="color: #000000; font-size: 20px;">Write whatever you want in the Custom text editor and enjoy our plugin!</span></p><p>&nbsp;</p>';
		break;
	    case 'it_IT':
		$welcome_text = '<h1 style="text-align: center;"><span style="color: #000000; font-size: 20px;">Questo &egrave; un esempio di popup.</span></h1><p style="text-align: center;">&nbsp;</p><p style="text-align: center;"><span style="color: #000000; font-size: 20px;">Le impostazioni base per iniziare sono: Altezza popup, Tempo popup, Prossima visualizzazione, Colore bordo, Colore sfondo.</span></p><p style="text-align: center;">&nbsp;</p><p style="text-align: center;"><span style="color: #000000; font-size: 20px;">Scrivi qualunque cosa vuoi nell&#39;editor di testo di wordpress e buon lavoro!</span></p><p style="text-align: center;">&nbsp;</p>';
		break;
	    default:
		$welcome_text = '<h1 style="text-align: center;"><span style="color: #000000; font-size: 20px;">Hello, this is a pop-up sample.</span></h1><p style="text-align: center;"><span style="color: #000000; font-size: 20px;">The basic stetting to get started are: Popup height, Popup time, Next visualization, Popup border color, Popup background.</span></p><p style="text-align: center;"><span style="color: #000000; font-size: 20px;">Write whatever you want in the Custom text editor and enjoy our plugin!</span></p><p>&nbsp;</p>';
	}
	ipp_itro_update_field('custom_html', $welcome_text);

	ipp_update_option('sample_popup', 'done');
    }
}

/* --------------------------CHECK THE PLUGIN VERSION */

function ipp_check_ver() {
    ob_start();
...
function ipp_display_popup() {
    ...
    switch (ipp_itro_get_option('users_display')){
        case 'logged':
    ...
    $selected_page_id = json_decode(ipp_itro_get_option('selected_page_id'));
    $id_match = NULL;
    ...
    switch (ipp_itro_get_option('page_selection')) {
	case 'some':
	    ...
	    if ((is_front_page() && ipp_itro_get_option('blog_home') == 'yes') || (is_home() && ipp_itro_get_option('blog_home') == 'yes')) {
		$id_match++;
	    }
...
function itro_check_selected_id($id_to_check) {
    if (ipp_itro_get_option('selected_page_id') != NULL) {
	$selected_page_id = json_decode(ipp_itro_get_option('selected_page_id'));
	$id_match = NULL;
	if (isset($selected_page_id)) {
	    foreach ($selected_page_id as $single_id) {
...
function ipp_itro_get_serverinfo() {
    global $wpdb;
    global $wp_version;
