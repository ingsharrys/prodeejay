...
global $wpdb;
define('IPP_OPTION_TABLE_NAME', $wpdb->prefix . 'itro_plugin_option');
define('IPP_FIELD_TABLE_NAME', $wpdb->prefix . 'itro_plugin_field');

/* -------Create plugin tables */

function ipp_itro_db_init() {
    global $wpdb;
    /* ------------------Option table */
    $option_table_name = IPP_OPTION_TABLE_NAME;
    ...
    /* --------------Custom field table */
    $field_table_name = IPP_FIELD_TABLE_NAME;
...
function ipp_update_db() {
	...
	if (ipp_itro_get_option('preview_id') != NULL) {
	    wp_delete_post(ipp_itro_get_option('preview_id'), true);
	}
    }
    // added the text for countdown 
    if (version_compare($prev_ver, 4.91) == -1) {
	ipp_update_option('countdown_text', 'This popup will close in:');
    }
    // reset the popup height after error for data sanitization
    if (version_compare($prev_ver, 4.95) == -1) {
	if (ipp_itro_get_option('select_popup_height') == 'px' && ipp_itro_get_option('px_popup_height') == 0) {
...
function ipp_update_option($opt_name, $opt_val) {
...
function ipp_itro_get_option($opt_name) {
    global $wpdb;
    $option_table_name = IPP_OPTION_TABLE_NAME;
...
function ipp_itro_update_field($field_name, $field_value) {
    global $wpdb;
    $field_table_name = IPP_FIELD_TABLE_NAME;
...
function ipp_itro_get_field($field_name) {
    global $wpdb;
    $field_table_name = IPP_FIELD_TABLE_NAME;
