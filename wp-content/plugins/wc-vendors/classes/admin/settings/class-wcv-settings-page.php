...
if ( ! class_exists( 'WCVendors_Settings_Page', false ) ) :
    ...
    abstract class WCVendors_Settings_Page {
        ...
        public function output_sections() {
            ...
            do_action( 'wcvendors_sections_' . $this->id . '_' . $current_section . '_start' );
            ...
            foreach ( $sections as $id => $label ) {
                echo '<li><a href="' . esc_url( admin_url( 'admin.php?page=wcv-settings&tab=' . $this->id . '&section=' . sanitize_title( $id ) ) ) . '" class="' . ( $current_section === $id ? 'current' : '' ) . '">' . esc_html( $label ) . '</a> ' . ( end( $array_keys ) === $id ? '' : '|' ) . ' </li>';
            }

            echo '</ul><br class="clear" />';
            do_action( 'wcvendors_sections_' . $this->id . '_' . $current_section . '_end' );
        }
