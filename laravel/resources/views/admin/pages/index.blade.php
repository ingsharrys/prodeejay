...
@section('content')
<p style="margin:0 0 16px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
...
<table class="tabla">
    <thead><tr><th>Página</th><th>URL</th><th>Idiomas</th><th>Estado</th><th></th></tr></thead>
    <tbody>
        @forelse ($pages as $p)
            <tr>
                <td>
                ...
                <td><a href="{{ url('/p/' . $p->slug) }}" target="_blank" style="color:#1db954;">/p/{{ $p->slug }}</a></td>
                <td style="font-size:12px;color:#b3b3b3;">ES @if($p->title_en || $p->content_en) · EN @endif</td>
                <td>
